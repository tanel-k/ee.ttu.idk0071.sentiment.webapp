...
define('resources/index',['exports'], function (exports) {
  ...
  function configure(config) {
    config.globalResources(['./value-converters/filter-by', './value-converters/order-by', './elements/graphs/pie-chart']);
  }
});
define('containers/lookup-detail/lookup-detail',['exports', 'aurelia-framework', 'aurelia-router', '../../gateways/data/data-api'], function (exports, _aureliaFramework, _aureliaRouter, _dataApi) {
  'use strict';
  ...
  var LookupDetail = exports.LookupDetail = (_dec = (0, _aureliaFramework.inject)(_dataApi.DataAPI, _aureliaRouter.Router), _dec(_class = function () {
    function LookupDetail(api, router) {
    ...
    LookupDetail.prototype.registerCompleteDomainLookup = function registerCompleteDomainLookup(id) {
      ...
      this.api.fetchDomainLookup(id).then(function (cpltDomainLookup) {
        _this3.completeDomainLookups.push(extendCompleteDomainLookup(flattenDomainLookup(cpltDomainLookup)));
      });
    };
...
define('containers/lookup-form/lookup-form',['exports', 'aurelia-framework', 'aurelia-router', 'aurelia-dialog', 'blockUI', '../../gateways/data/data-api', '../../lib/utils', './dialogs/lookup-submitted', 'jquery'], function (exports, _aureliaFramework, _aureliaRouter, _aureliaDialog, _blockUI, _dataApi, _utils, _lookupSubmitted) {
  'use strict';
  ...
  var LookupForm = exports.LookupForm = (_dec = (0, _aureliaFramework.inject)(_dataApi.DataAPI, _aureliaRouter.Router, _aureliaDialog.DialogService), _dec(_class = function () {
    ...
    LookupForm.prototype.created = function created() {
      ...
      this.api.fetchDomains().then(function (domains) {
        _this.domainOptions = domains.map(function (d) {
          return {
            label: d.name,
            value: d.code
          };
    ...
    LookupForm.prototype.performLookup = function performLookup() {
      ...
      this.api.postLookup({ entityName: entityName, domainIds: domainIds }).then(function (lookupResult) {
        ...
        _this2.domainIds = [];
        _this2.entityName = '';
        _this2.openResultDialog({ lookupResult: lookupResult, entityName: entityName });
      }).catch(function (err) {});
    };
...
define('text!containers/lookup-detail/lookup-detail.html', ['module'], function(module) { module.exports = "<template><div class=\"snapshot-report\"><div class=\"row\"><div class=\"col-md-12\"><h1><em>${lookupData.lookupEntity.name}</em> lookup <small>code: ${lookupData.id}</small></h1></div></div><div class=\"row\"><div class=\"${hasCompleteLookups ? 'col-md-6' : 'col-md-12'}\" if.bind=\"incompleteDomainLookups.length > 0\"><h3>Incomplete lookups</h3><div repeat.for=\"domainLookup of incompleteDomainLookups | orderBy:'domainName':'asc'\" class=\"panel ${domainLookup.currStateName === 'Queued' ? 'panel-default' : 'panel-warning'}\"><div class=\"panel-heading\">${domainLookup.domain.name}</div><div class=\"panel-body\"><p if.bind=\"domainLookup.currStateName === 'In progress'\">This lookup is being processed</p><p if.bind=\"domainLookup.currStateName === 'Queued'\">This lookup is waiting for a processor</p></div></div></div><div class=\"${hasIncompleteLookups ? 'col-md-6' : 'col-md-12'}\" if.bind=\"completeDomainLookups.length > 0\"><h3>Complete lookups</h3><div repeat.for=\"domainLookup of completeDomainLookups | orderBy:'isSuccess':'asc' | orderBy:'domainName':'asc'\" class=\"panel ${domainLookup.isError ? 'panel-danger' : 'panel-success'}\"><div class=\"panel-heading\">${domainLookup.domain.name}</div><div class=\"panel-body\"><div if.bind=\"domainLookup.isSuccess === true\"><div class=\"row\"><div class=\"${hasIncompleteLookups ? 'col-md-12' : 'col-md-6'}\"><div class=\"panel panel-default\"><div class=\"panel-heading\">Results</div><div class=\"panel-body\"><div>Total text sources processed: <strong>${domainLookup.totalCount}</strong></div><div>Positive sources: <strong>${domainLookup.positiveCount}</strong></div><div>Neutral sources: <strong>${domainLookup.neutralCount}</strong></div><div>Negative sources: <strong>${domainLookup.negativeCount}</strong></div></div></div></div><div class=\"${hasIncompleteLookups ? 'col-md-12' : 'col-md-6'}\"><pie-chart items.bind=\"domainLookup.chartDataItems\" width=\"325\" height=\"300\"></pie-chart></div></div></div><div if.bind=\"domainLookup.isError === true\"><p>An error occurred while processing this lookup</p></div></div></div></div></div></div></template>"; });
